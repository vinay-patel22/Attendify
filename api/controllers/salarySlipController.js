...
exports.generateSalarySlips = async (req, res) => {
    try {
        ...
        const attendanceData = await Attendance.aggregate([
            ...
            },
            {
                $group: {
                    _id: "$employeeId",
                    present: {
                        $sum: { $cond: { if: { $eq: ["$status", "present"] }, then: 1, else: 0 } },
                    },
                    absent: {
                        $sum: { $cond: { if: { $eq: ["$status", "absent"] }, then: 1, else: 0 } },
                    },
                    halfday: {
                        $sum: { $cond: { if: { $eq: ["$status", "halfday"] }, then: 1, else: 0 } },
                    },
                    holiday: {
                        $sum: { $cond: { if: { $eq: ["$status", "holiday"] }, then: 1, else: 0 } },
                    },
                },
            },
        ]);
        ...
        for (const attendance of attendanceData) {
            ...
            const earnedSalary = attendance.present * perDaySalary + attendance.halfday * perDaySalary * 0.5;
